...
use wallet_adapter::{
    ...
    Cluster, SendOptions, WalletAdapter, WalletError, WalletResult,
};

use crate::{AccountState, FetchReq};
...
pub async fn get_balance(address: &str, endpoint: &str) -> WalletResult<String> {
...
pub async fn send_sol_req(
    ...
    let blockhash = get_blockhash(endpoint).await?;
...
pub async fn request_airdrop(lamports: u64, address: &str, endpoint: &str) -> WalletResult<()> {
...
pub async fn accounts_runner(address: &str, endpoint: &str) -> WalletResult<AccountState> {
    let balance = crate::get_balance(address, endpoint).await?;
...
pub(crate) async fn get_blockhash(endpoint: &str) -> WalletResult<solana_sdk::hash::Hash> {
    let options = jzon::object! {
        "id":1,
        "jsonrpc":"2.0",
